...
const GERENTES = [
  "Ana Souza", "Carlos Lima", "Daniel Rocha", "Fernanda Melo",
  "Gustavo Pires", "Helena Brito", "Joaquim Sá", "Larissa Vieira",
];
const CANAIS = [
  "PAP Próprio", "PAP Parceiro", "Loja Própria", "Loja Parceira",
  "Televendas", "Digital", "Field Sales", "Revenda 5G",
  "B2B Indireto", "Eventos", "WhatsApp Bot", "Porta a Porta NE",
];
const NICHOS = ["Residencial", "PME", "Corporativo", "Combo Família", "Pré-pago 5G"];
const UFS = ["CE", "RN", "PB", "PE", "AL", "SE", "BA", "PI", "MA"];
const CIDADES = ["Fortaleza", "Sobral", "Natal", "Mossoró", "João Pessoa", "Recife", "Maceió", "Aracaju", "Teresina", "São Luís"];
...
function filterFactor(f: SalesFilters): number {
  ...
  const periodMul: Record<string, number> = {
    mes_atual: 1, mes_anterior: 1, "7d": 0.25, "30d": 1, "90d": 2.9, ano: 11,
  };
...
function kpisBL(f: SalesFilters): KpiBlock[] {
  ...
  return [
    { label: "Vendas Criadas", value: criadas, meta: Math.round(175_000 * factor), delta: 6.3, available: true, helper: tag },
    { label: "Vendas Efetivadas", value: efet, meta: Math.round(130_000 * factor), delta: 4.2, available: true, helper: `Efetivados x Criados: ${efetXCri}%`.replace(".", ",") },
    { label: "Vendas Instaladas", value: inst, meta: Math.round(122_000 * factor), delta: -2.1, available: true, helper: `Instalados x Efetivados: ${instXEfet}%`.replace(".", ",") },
    { label: "Ticket de Entrada", value: +ticket.toFixed(1), meta: 105, unit: "currency", delta: 4.2, available: true, helper: `Faturamento entrada: R$ ${fatEntrada.toFixed(1)}M`.replace(".", ",") },
    { label: "Churn Safra", value: +churn.toFixed(1), meta: 5.5, unit: "percent", delta: -0.8, available: true, helper: tag },
  ];
}
...
function kpis5G(f: SalesFilters): KpiBlock[] {
  ...
  return [
    { label: "Vendas Ativadas 5G", value: Math.round(41_320 * factor), meta: Math.round(38_000 * factor), delta: 8.7, available: true, helper: "Chip pago/grátis: sem acesso" },
    blocked("% Portabilidade (Concluída x Ativ.)"),
    blocked("% Portabilidade (Concluída x Solic.)"),
    { label: "Ticket Médio Entrada 5G", value: 49.9, meta: 47, unit: "currency", delta: 6.1, available: true, helper: "Fat. entrada: R$ 2,06M" },
...
function freeSeries(): Record<string, { mes: string; valor: number }[]> {
  ...
  for (const nome of FREE) {
    ...
    out[nome] = MESES.map((mes, i) => {
      const base = isPct ? 40 + Math.sin(i / 2 + nome.length) * 8 : 8000 + Math.sin(i / 2 + nome.length) * 1500 + i * 200;

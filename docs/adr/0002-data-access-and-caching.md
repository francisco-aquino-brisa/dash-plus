...
- **Small datasets** (e.g. the curated per-city/month `indicadores_cidades`, a few thousand rows): fetch the broad slice once _without_ the dimensional filters in the `WHERE`, cache it, and apply filters/aggregations in JS. Switching filters costs zero extra queries.
...
- **On**: a cheap watermark probe runs every ~1 minute and triggers the heavy query _only_ when the source has actually advanced.

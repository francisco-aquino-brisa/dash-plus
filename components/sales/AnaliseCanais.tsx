...
function Delta({ v }: { v: number }) {
  const up = v >= 0;
  return (
    <span className={cn("flex items-center justify-end gap-0.5 tabular-nums", up ? "text-success" : "text-destructive")}>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={`${r.canal}-${i}`} className="border-t border-border/40 hover:bg-secondary/40">
                ...
                <td className="px-3 py-2 text-right font-semibold tabular-nums">{formatNumber(r.mediaDia)}</td>
                <td className="px-3 py-2 text-right"><Delta v={r.vsMesAnterior} /></td>
                <td className="px-3 py-2 text-right"><Delta v={r.vsSemanaAnterior} /></td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <section className="rounded-2xl border border-border bg-card/40 p-5 shadow-elegant backdrop-blur">
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Média por dia útil e variação vs mês/semana anterior</p>
        </div>
